...
class MinimalOpcUaClient(Node):
    def __init__(self):
        ...
        self.activate_robot_pub = self.create_publisher(
            Bool,
            "activate_robot_signal",
            qos_profile=QoSProfile(
                depth=1,  # Small queue to prevent message buildup
        ...
        self.get_logger().info(
            "Created reliable publisher for activate_robot_signal topic"
        )

        # --- NEW: Pulse management variables ---
        self.pulse_lock = threading.Lock()
        self.pulse_sent_timestamp = 0
        self.min_pulse_interval = 2.0  # Minimum 2 seconds between pulses
    ...
    def joint_state_callback(self, msg: JointState):
        ...
        # Extract the first 7 joint positions (joint2 through joint7, indices 0-6)
        if len(msg.position) >= 7:
            self.latest_joint_positions = list(msg.position[:7])
            self.joint_states_received_first_time = True
            self.get_logger().debug(
                f"Received 7 joint positions: {self.latest_joint_positions}"
            )
    ...
    def _publish_activate_robot_pulse(self):
        """Publish a single reliable pulse (True) to ROS2 topic when activation occurs."""
        with self.pulse_lock:
            current_time = time.time()

            # Prevent rapid-fire pulses (debounce)
            if current_time - self.pulse_sent_timestamp < self.min_pulse_interval:
                self.get_logger().warning(
                    f"Pulse ignored - too soon after last pulse "
                    f"({current_time - self.pulse_sent_timestamp:.2f}s ago)"
                )
                return

            # Create and publish the activation pulse
            msg = Bool()
            msg.data = True

            # Publish multiple times to ensure delivery in multi-node environment
            for i in range(3):
                self.activate_robot_pub.publish(msg)
                if i < 2:  # Don't sleep after the last publish
                    time.sleep(0.05)  # 50ms between publishes

            self.pulse_sent_timestamp = current_time

            self.get_logger().info(
                f"ACTIVATION PULSE SENT at {current_time:.3f} - Published 3 times to ensure delivery"
            )

            # Optional: Send a clear "end of pulse" signal after a short delay
            self.create_timer(0.2, self._send_pulse_end)

    def _send_pulse_end(self):
        """Send a False signal to clearly mark end of pulse."""
        msg = Bool()
        msg.data = False
        self.activate_robot_pub.publish(msg)
        self.get_logger().debug("Sent pulse end signal (False)")
