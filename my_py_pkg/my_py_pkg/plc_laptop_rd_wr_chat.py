...
def read_data_from_plc(plc, db_number, start_byte, size):
    ...
    try:
        # Read the specified number of bytes from the Data Block (DB)
...
def write_data_to_plc(plc, db_number, start_byte, data):
    """Writes data to a specified data block in the PLC."""
    try:
        # Write data to the PLC (Data Block)
        plc.write_area(snap7.types.Areas.DB, db_number, start_byte, data)
        print("Successfully wrote data to PLC.")
    except Exception as e:
...
def main():
    ...
    # Example: Write data to the same data block (DB1, start byte 0)
    new_value = 12345  # Example new value to write (you can change this)
    new_data = new_value.to_bytes(
        2, byteorder="big", signed=True
    )  # Convert to 2-byte signed integer
    write_data_to_plc(plc, db_number, start_byte, new_data)
